...
knitr::opts_knit$set(root.dir = 'C:/Users/UOM74593User/OneDrive - The University of Melbourne/MyPhDProject_in_R_GIT')
```
...
file07 <- list.files(path = "F:/RQ1_RQ2/data/rawdata/2007", pattern = ".las$")
#cat07 <- catalog("2007", list.files(path = "F:/RQ1_RQ2/data/rawdata/2007", pattern = ".las$"), full.name = TRUE, recursive = TRUE)
# cat07 <- catalog("F:/RQ1_RQ2/data/rawdata/2007", recursive = TRUE)
# cat07 <- catalog("J:/Merge_2007", recursive = FALSE)
cat07 <- catalog("./data/lidar/merge_07", recursive = FALSE)
class(cat07)
plot(cat07)
cat07@crs = sp::CRS("+proj=utm +zone=55 +south +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m")# Providing the coordinates system with a proj4 string enable to display the catalog on an interactive map
plot(cat07)
# spdf <- SpatialPolygonsDataFrame(cat07@crs)
# writeOGR(cat07@crs, "./data/lidar/merge_07", 'cat07', driver = "ESRI Shapefile")
# how to write catalog as a shapefile
# cat12 <- catalog("F:/RQ1_RQ2/data/rawdata/2012")
cat12 <- catalog("./data/lidar/lidar_12", recursive = FALSE)# Extra parameters to list.files. Typically 'recursive = TRUE'.
cat12@crs = sp::CRS("+proj=utm +zone=55 +south +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m")
plot(cat12)
lidar16 <- list.files(path = "F:/RQ1_RQ2/data/rawdata/2016", pattern = ".las$")
# cat16 <- catalog("F:/RQ1_RQ2/data/rawdata/2016")
cat16 <- catalog("./data/lidar/lidar_16", recursive = FALSE)
cat16 <- data.frame(cat16)
#Error in as.data.frame.default(x[[i]], optional = TRUE) : cannot coerce class "structure("LAScatalog", package = "lidR")" to a data.frame
cat16@crs = sp::CRS("+proj=utm +zone=55 +south +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m")
plot(cat16) # plot location or site boundary can be added in this plot, try it
...
## Error in `$<-`(`*tmp*`, year, value = 2007) : no method for assigning subsets of this S4 class
...
siteloc <- readOGR("./GIS_data_display", "SiteBoundaryAll_Centroid")
str(siteloc)
roinames <- as.vector(siteloc@data$SiteID) # to make the vector for fulfilling the parameter of catalog_queries
class(roinames)
roinames12 <- roinames[c(6, 8, 9, 12)] # select only few of the plotIDs

X = siteloc$POINT_X # x coordinates
Y = siteloc$POINT_Y # Y coordinates

### Clipping the lidar data with plot location for 100*100 meter rectangle##
clip07 = cat07%>% catalog_queries(X, Y, 100, 100, buffer=10, roinames, filter = "-drop_z_below 0")
clip12 = cat12%>% catalog_queries(X, Y, 100, 100, buffer=10, roinames12, filter = "-drop_z_below 0")
#Progress: 100%ize 4 but maximum size is 17 (recycled leaving remainder of 1 items)12_MS is outside the catalog.11_MS is outside the catalog.1_HS is outside the catalog.4_HS is outside the catalog.12_MS is outside the catalog.1_HS is outside the catalog.11_MS is outside the catalog.1_HS is outside the catalog.12_MS is outside the catalog.11_MS is outside the catalog.1_HS is outside the catalog.4_HS is outside the catalog.12_MS is outside the catalog.
clip16 = cat16%>% catalog_queries(X, Y, 50, 50, buffer=10, roinames, filter = "-drop_z_below 0")
names(clip16) # provides only the name of files
# Progress: 100%Error: unused argument (mc.cores = 8), remove this arguement from the lidR function

## Saving the clip file as RDS for serialization##
rds07 <- saveRDS(clip07, file = "./data/lidar/merge_07/plotclip07.rds")
rds12 <- saveRDS(clip12, file = "./data/lidar/lidar_12/plotclip12.rds")
rds16 <- saveRDS(clip16, file = "./data/lidar/lidar_16/plotclip16.rds")

## Reading the RDS file#
RDS07 <- readRDS(file = "./data/lidar/merge_07/plotclip07.rds")
RDS12 <- readRDS(file = "./data/lidar/lidar_12/plotclip12.rds")
RDS16 <- readRDS(file = "./data/lidar/lidar_16/plotclip16.rds")
```
...
for (i in 1:length(mylist16)) {}# apply loop function for all the files in the folder
{
  ...
  outname <- paste0('./data/lidar/lidar_16/clipraw/', file_name, "_16.las")
  # dir.create(paste0("F:/RQ1_RQ2/metrics/"), showWarnings = FALSE)
  # write.csv (answer, file.path(paste0("F:/RQ1_RQ2/metrics/"), outname))
